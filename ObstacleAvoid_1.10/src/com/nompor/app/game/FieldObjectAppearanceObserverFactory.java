...
import com.nompor.app.game.enemy.Bird;
import com.nompor.app.game.enemy.GreenSlime;
import com.nompor.app.game.enemy.NeedleLeaf;
import com.nompor.app.game.enemy.RedSlime;
import com.nompor.app.game.enemy.Slime;
...
import com.nompor.gtk.fx.image.ImageUtilFX;
...
public class FieldObjectAppearanceObserverFactory {
	...
	public static Enemy createEnemy(FieldObject type, int x, int y) {
		...
		case GREEN_SLIME:{
			...
			//ImageManagerのキャッシュ機能を利用して必要なら新しい画像をキャッシュしておく
			Image slimeImage = GTKManagerFX.getImageManager().getCache(FieldObject.GREEN_SLIME);
			if ( slimeImage == null ) {
				//元のスライム画像を緑色に変換し、キャッシュする
				slimeImage = ImageUtilFX.toEffectorImage(AppManager.getImage("slime"), new ColorAdjust(-0.6, 0, 0, 0));
				GTKManagerFX.getImageManager().setCache(FieldObject.GREEN_SLIME, slimeImage);
			}

			ImageAnimationView img = createImageAnimationView(Duration.millis(200), slimeImage,50,50);//表示ノード
			img.setCycleCount(Animation.INDEFINITE);
			img.setIndex(0);
			Rectangle r = new Rectangle(x+8,y+26,35,20);//当たり判定
			img.setTranslateX(x);
			img.setTranslateY(y);
			e = new GreenSlime(img, r);
			break;
		}
		case RED_SLIME:{
			//赤スライム
			...
			//ImageManagerのキャッシュ機能を利用して必要なら新しい画像をキャッシュしておく
			Image slimeImage = GTKManagerFX.getImageManager().getCache(FieldObject.RED_SLIME);
			if ( slimeImage == null ) {
				//元のスライム画像を赤色に変換し、キャッシュする
				slimeImage = ImageUtilFX.toEffectorImage(AppManager.getImage("slime"), new ColorAdjust(0.6, 0, 0, 0));
				GTKManagerFX.getImageManager().setCache(FieldObject.RED_SLIME, slimeImage);
			}
			...
			img.setTranslateX(x);
			img.setTranslateY(y);
			e = new RedSlime(img, r);
			break;
		}
		case NEEDLE_LEAF:{
			//ジャンプする棘植物
			ImageAnimationView img = createImageAnimationView(Duration.millis(1000), AppManager.getImage("toge"),50,50);//表示ノード
			img.setCycleCount(Animation.INDEFINITE);
			img.setIndex(0);
			Rectangle r = new Rectangle(x+13,y+22,28,26);//当たり判定
			img.setTranslateX(x);
			img.setTranslateY(y);
			e = new NeedleLeaf(img, r);
			break;
		}
		default:break;
		}
